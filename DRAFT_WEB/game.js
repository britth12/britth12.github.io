...
// Draws the two blue accent lines at top and bottom — shared by all menu screens
function drawBorder() {
...
function plankW(key) {
  ...
  return img?.naturalWidth ? Math.round(PLANK_H*img.naturalWidth/img.naturalHeight) : 120;
}
...
const GAME_DURATION=20, SPAWN_MS=1000, MAX_NUMBERS=5, MAX_SCORE=100;
...
function spawnNumber() {
  ...
  for (let attempt=0; attempt<10; attempt++) {
    ...
    if (!numbers.some(n=>Math.abs(n.x-nx)<MIN_SPAWN_DIST&&Math.abs(n.y-ny)<MIN_SPAWN_DIST)) {
...
function update() {
  ...
  if ((keys['ArrowUp']||keys['KeyW']||keys['Space'])&&player.onGround) { player.vy=JUMP_FORCE; player.onGround=false; }
...
function drawInstructionsScreen() {
  ...
  drawStr(W/2, topY+lineGap*2, 'AS YOU CAN IN TWENTY SECONDS',   instructLine,'center');
  ctx.globalAlpha=0.6+0.4*Math.sin(Date.now()*0.004);
