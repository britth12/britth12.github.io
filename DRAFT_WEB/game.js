...
function buildWizSheet(raw, cellWKey, cellHKey, sheetKey) {
  if (!raw?.naturalWidth) { assets[sheetKey] = null; return false; }
  ...
  frameCvs.forEach((fc,i) => { const cropW=Math.min(cw,fc.width-ul); sx.drawImage(fc,ul,ut,cropW,ch,i*CW+PAD,PAD,TW,TH); });
...
function plankW(key) {
  ...
  return img?.naturalWidth ? Math.round(plankH*img.naturalWidth/img.naturalHeight) : 120;
}

const platforms = [
  {x:0,   y:525, h:75,    plank:null},
  {x:80,  y:415, h:plankH, plank:'plank1'},
  {x:480, y:415, h:plankH, plank:'plank3'},
  {x:900, y:415, h:plankH, plank:'plank5'},
  {x:180, y:315, h:plankH, plank:'plank2'},
  {x:520, y:310, h:plankH, plank:'plank4'},
  {x:870, y:310, h:plankH, plank:'plank1'},
  {x:50,  y:210, h:plankH, plank:'plank3'},
  {x:440, y:205, h:plankH, plank:'plank5'},
  {x:920, y:210, h:plankH, plank:'plank2'},
  {x:260, y:115, h:plankH, plank:'plank4'},
  {x:740, y:115, h:plankH, plank:'plank3'},
];
...
const exitFloor=525, exitTop=exitFloor-66;

function drawExit() {
  ctx.fillStyle='rgba(220,40,40,0.85)';
  ctx.fillRect(W-2, exitTop, 2, exitFloor-exitTop+exitDoorExtra);
...
const spawnY=525-66;
const player={x:586, y:spawnY, w:42, h:66, vx:0, vy:0, onGround:false, facing:1, dropThrough:0};

function resetPlayer() {
  Object.assign(player,{x:586,y:spawnY,vx:0,vy:0,onGround:false,facing:1,dropThrough:0});
...
function startMusic() {
  if (!musicMuted) { music.volume=0.4; music.play().catch(()=>{}); }
}
...
function update() {
  ...
  player.vx=gL?-playerSpeed:gR?playerSpeed:0;
...
function fitCanvas() {
  ...
  const scale=Math.min(1.0,(window.innerWidth-64)/1200,(window.innerHeight-64)/600);
